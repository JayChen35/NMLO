...
def relu(feature_map):  
    ...
    return np.maximum(zeros, feature_map)
...
def pixels2img(pixels):
    ...
    sqrt = math.sqrt(len(pixels))
    assert(sqrt == int(sqrt))
    sqrt = int(sqrt)
    img = pixels.reshape((sqrt, sqrt))
    return img
...
def preprocess(all_pixels):
    out = np.zeros((len(all_pixels), 288), dtype=np.float64)
    print("Num images:", len(all_pixels))
    for i in range(len(all_pixels)):
        if i % 100 == 0:
            print(i)
        pixels = all_pixels[i]
        img = pixels2img(pixels)
        features = conv_layer(img)
        out[i] = features.reshape(-1,)
    print(out.shape)
    return out
